...
fields \citep{Errington2014, Klein2014, OSC2015, Ebersole2016, Camerer2016,
...
% \emph{statistical} significance of both the original and replication
% study,
%  compatibility of their effect estimates, and meta-analysis of
% the effect estimates. \emph{Many of the replication projects listed above also
...
% In order to address \emph{this problem}, a new method has recently been
...
% make the original study non-significant. \emph{This approach penalizes
...
% \emph{study is achieved if $z_r \geq z_\alpha$, which}
% is equivalent to the \emph{condition}
...
% positive) relative effect size $d$. \emph{However, declaring replication
...
% \emph{In this paper we first review the \citet{held2020} approach for
...
% \end{knitrout}
% \caption{Example of the assessment of replication success. The original study from \citet{Pyc2010} has effect estimate  $\hat \theta_o=
%   0.4$ on Fisher's $z$ scale
%   (95\% CI from $0.05$ to $0.74$) and one-sided $p$-value
%   $p_o = 0.011$.
%   The left part of the figure illustrates the \emph{reverse-Bayes derivation of the sufficiently sceptical prior
%     based on}
%   the original study result and the posterior with lower credible limit fixed at zero. \emph{The comparison} of the sufficiently sceptical prior with the replication study result ($\hat \theta_r = 0.15$, 95\% CI from 0.04 to 0.26, $p_r = 0.004$)
%   in the right part of the figure \emph{is used to assess potential prior-data conflict.}
%   \label{fig:fig1}}
% \end{center}
% \end{figure}
% \end{center}
...
% one-sided level $\alpha=0.025$ is
...
% \emph{disprove} the sceptic. Conflict is quantified by a prior-predictive
...
% sceptical prior (one-sided $p_{\mbox{\scriptsize Box}}=0.31$).
% We are thus not able to declare replication success at
% level $2.5$\%.

% \emph{The actual value of $p_{\mbox{\scriptsize
...
% The requirement \eqref{eq:extrinsic.p} can be assessed for \emph{any} value of
% \emph{${{\alpha_S}} > \max\{p_o, p_r\}$} and of particular interest is the
...
% $p$-value turns out to be $p_S=0.11$.
...
% $p_S$ increases with \emph{decreasing} relative effect size
...
% \emph{achieve} replication success.  The second property guarantees that
...
% \emph{The derivation is based on a property of the required
...
% significant \emph{($p_o=\alpha$)}, replication success is only possible if the replication
% effect estimate is larger than the original one \emph{($d > 1$)}.\\


% \subsection{Relative effect size}\label{sec:res}
% Without loss of generality we \emph{now} assume that $\hat \theta_o > 0$ and that
...
% sample size leads to a more stringent success requirement \emph{for $z_r$ and the corresponding} replication
% $p$-value $p_r$.
...
%   \dinfty = 1/\sqrt{K (K-1)}
% \end{equation}
% \emph{is the {\it limiting relative effect size}.}
...
% \emph{In contrast}, the corresponding criterion \eqref{eq:dSig} \emph{of the
% two-trials rule}
...
% $K = \varphi$ where
% $\varphi = (\sqrt{5}+1)/2 \approx 1.62$ is
% known as the golden ratio. Solving for $z_{\alpha_S}$ gives
% \emph{$z_{\alpha_S} = z_\alpha / \sqrt{\varphi}$
...
% the golden level is $\alpha_S =0.062$.
% In the introductory example
% shown in Figure \ref{fig:fig1},
% the sceptical $p$-value is $p_S=0.11 > 0.062$, so
% the replication  of the \citet{Pyc2010} study was not successful.




% \emph{The golden level \eqref{eq:golden} is derived from~\eqref{eq:smallestpossible} with $\dinfty = 1$.
...
% $\tilde{p}_S=0.061$ and does not lead to replication
% success at \emph{any level $\alpha<0.061$, including} the
...
% 0.062$) and the two-trials rule ($p_o \leq 0.025$ and $p_r \leq 0.025$).
% The \emph{dotted} areas indicate that success is
...
% \emph{The dashed black line in the left plot indicates the limiting relative
% effect size $\dinfty$.}
% }
% \label{fig:fig2}
% \end{figure}
...
% $\alpha=0.025$, original studies with a $p$-value larger than
% $0.006$ will require a smaller relative effect
...
% investigate the \emph{overall} Type-I error rate (Section \ref{sec:T1E}) and the project power
% (Section \ref{sec:PP})
% \citep{Maca2002} \emph{over both studies in combination for fixed relative sample size $c$}.
...
% \end{knitrout}
% \end{center}
% \caption{\emph{Conditional} power as a function of the {one-sided}
%   $p$-value of the original study with \emph{relative} sample size
%   $c=1$ (left) and $c=5$ (right). Shown is
%   conditional power \emph{assuming the unknown parameter is equal to
%   the original effect estimate (solid)} \emph{and conditional
%     power based on 20\% shrinkage of the original
%     effect estimate (dashed)} for the two-trials rule (2TR) at level
%   $\alpha = 0.025$ and for replication success
%   % (RS)
%   at the
...
% \emph{\citep[see][Section 4 for details]{held2020}} at the golden and at
% the nominal level with the power of the two-trials rule for relative
% sample size $c = 1$ (left) and $c = 5$ (right)
% as a function of the one-sided $p$-value \emph{$p_o$} from the original
% study.  \emph{Shown is the conditional power assuming the unknown parameter
...
% impossible to power a replication study when $p_o > 0.025$.
% \emph{The same applies for replication success at the nominal level,
% where the power is zero for any $p_o > 0.025$, regardless
% of the replication sample size.
% This is different for the golden level,
% where the conditional power of an original study with $0.025 < p_o
% < 0.062$ is low, but not zero.}
% \emph{However, if the original $p$-value $p_o$ is slightly smaller than
% $0.025$, the two-trials rule has a larger power, both for $c=1$ and $c=5$. }
% \emph{But if the original $p$-value is sufficiently
% small ($p_o < 0.006$ for $c=1$), the power for replication success at the golden level is larger
...
%   smaller threshold $1-\Phi(z_\alpha \sqrt{\varphi})$ which is $0.006$ for  $\alpha=0.025$. Only if $p_o < 0.006$ will the conditional power
%   for replication success attain 100\% for $c \to \infty$. This further highlights the stringency
%   of the nominal level.  }

% \emph{The approach described so far takes the original study at
...
%   on 20\% shrinkage of the original effect estimate
%   which reduces the conditional power for all methods, especially for
%   a relative sample size $c = 1$.  Conditional power for
%   replication success at the golden level can now be pushed towards 100\%
%   only for $p_o < 0.018$, which can be derived by
%   solving \eqref{eq:alphaPrime} for $\alpha$ with
%   $\alpha'=0.025$ and $\dinfty=0.8$.  To be able to
...
%   from $\alpha=0.025$ to $\alpha'=0.033$.
%   }

% \subsection{Overall Type-I error rate}\label{sec:T1E}
% \emph{The two studies are assumed to be independent with Type-I error
% rate fixed at $\alpha$ for each of them,} so the Type-I error rate of the
% two-trials rule \emph{over the entire project} is simply $\alpha^2$ for
...
% \emph{the relative sample size} $c$.
...
%  \right]\right\}/2.
% \end{eqnarray}
% For the golden level $\alpha_S =0.062$ at
% $\alpha = 0.025$, the Type-I error rate \eqref{eq:T1E}
% is $0.0515$\%, slightly less than the Type-I
% error rate $\alpha^2=0.0625$\% of the
% two-trials rule.  For comparison, the Type-I error rate at the nominal
% level $\alpha_S=0.025$ is
% $0.0022$\%, \emph{much} smaller than
% 0.0625\%.
...
% \caption{\emph{Overall} Type-I error rate (left) and project power (right) for fixed relative sample size $c$.
%   Results are given for replication success
%   % (RS)
%   at the nominal and golden
%   level and compared with the two-trials rule (2TR) at $\alpha = 0.025$.
%   \emph{The dashed darkgrey line is the project power at the golden level based on significant original studies ($p_o \leq 0.025$).}
%   The power of the original study is 90\%.}
% \label{fig:fig4}
% \end{figure}
...
% rate for $\alpha=0.025$ as a function of the relative sample
% size $c$.  It can be seen that the Type-I error \emph{of the replication success approach} decreases with
% increasing relative sample size $c$. \emph{This also follows from \eqref{eq:T1E0}
%   where
% $\P(z_r \geq \zrmin \given z_o, c, \alpha_S) = 1 - \Phi(\zrmin)$
...
% $0.0625$\%.  Although the Type-I error will
...
% golden level is smaller than $0.0625$\% for
% $c >0.85$.  Appropriate Type-I error control is
...
% {\centering \includegraphics[width=\maxwidth]{figure/fig5-1}
...
% \end{center}
% \caption{\emph{Overall Type-I error rate if the
%   replication sample size equal to the original study
%   ($c=1$).  The two-trials rule (2TR)
...
% \emph{Figure \ref{fig:fig5} compares for $c=1$ the Type-I error rate \eqref{eq:T1E}
...
%   if $\alpha < 0.058$. For $\alpha = 0.058$ the
%   Type-I error rate is equal to the
%   Type-I error rate $0.058^2 = 0.34\%$ of the
%   two-trials rule and for $\alpha > 0.058$ the
...
% two-trials rule is therefore $(1-\beta) \, \Phi(\sqrt{c} \mu - z_\alpha)$ \emph{and increases with increasing $c$.}
...
% \emph{The advantage in power stems partly from replication success still
...
%   $p_o \leq 0.025$), then the project power based on the
...
%   $p_o \leq 0.025$ may not reflect current practice in
%   large-scale replication projects. For example, 5 out of
%   143 replication studies considered in Section
...
% \emph{and} is bounded to the interval between minus one and one.
...
% $0.86$ in the Philosophy project. The degree of shrinkage is also
% reflected in the success rates (according to the two-trials rule and the replication
% success approach \emph{at the golden level}), which are around 30\% for the former and more than 70\%
% for the latter. The proportion of successful replications is similar for the
% two-trials rule and the replication success approach. \emph{In the Experimental
...
% {\centering \includegraphics[width=\maxwidth]{figure/fig6-1}
...
% black line indicates the \emph{limiting relative effect size}
...
% \emph{This analysis was based on the default choice $\dinfty=1$
...
%   $\alpha' \in (0.06,
%   0.022)$ for all four replication projects.
...
% {\centering \includegraphics[width=\maxwidth]{figure/fig7-1}
...
% gains in project power and controls the Type-I error rate \emph{at any
%   one-sided level $\alpha < 0.058$} if the
% replication sample size is not \emph{smaller than the original one}.
...
% \emph{The good performance has been recently confirmed by a comparison of different
...
% \emph{However, in extreme cases the performance of the sceptical $p$-value may be
...
% \emph{Throughout the paper we have assumed that the relative sample size is
