...
\begin{table}
  ...
  \caption{Subset of results from the ``Reproducibility Project: Cancer
    ...
    \textit{p}-value $p_{Q}$, and sceptical \textit{p}-value $p_{\mathrm{S}}$. A
    replication success (green) threshold of $\alpha = 5\%$ is used for the
    \textit{p}-values and $d_{\text{min}} = 1$ for the relative effect
    estimate.}
  \label{tab:RPCB}
...
smdr <- -3
so <- sr <- 1
ps2 <- pSceptical(zo = smdo/so, zr = smdr/sr, c = so^2/sr^2,
                  alternative = "two.sided", type = "nominal")
@
...
% to $\ps = \Sexpr{pFormat(ps2)}$, and the sceptical $p$-value continues to
...
\begin{align*}
  \tilde{p}_{\text{S}} =
  \begin{cases}
    \ps/2 & \text{if} ~\sign(\that_{o}) = \sign(\that_{r}) \\
    1 - \ps/2 & \text{if} ~ \sign(\that_{o}) \neq \sign(\that_{r}) \\
  \end{cases}
\end{align*}
