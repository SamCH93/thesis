...
The ability to trust scientific findings is essential for making evidence-based
...
merely by chance. For this reason, ``successful'' replication is often a
requirement for acceptance of newly proposed scientific theories (\eg{} a new
physical model). Yet, replication studies are not only important for
researchers, but also for decision makers and the general population, as the
implementation of policies based on scientific findings may also depend on
whether these findings can be replicated (\eg{} market approval of a drug). The
results from replication studies may thus also have real-world consequences, and
it is therefore important that the methodological aspects of replication
studies, such as their statistical design and analysis, are well understood.
...
on ``innovation'' rather than on ``repetition'' \citep{Makel2012, Martin2017,
  Coiera2021}. As a result, researchers often had no other choice than to focus
their efforts on producing novel and eye-catching research results in order to
build successful careers \citep{Binswanger2014, Moher2018}.
...
estimate. Typically, the available standard error $\sigma$ is only an estimate
of the true standard error. However, in most parts of this thesis it will be
assumed that $\sigma$ corresponds to the true standard error. This is the same
assumption as in ordinary fixed-effect/random-effects meta-analysis, and
typically reasonable if the sample size of the studies is not too small. Under
the assumption of (asymptotic) normality, confidence intervals for $\theta$ and
