...
studies are an essential part of the scientific process as they provide a means
...
estimate.\footnote{Typically, the available standard error $\sigma$ is only an
  ...
  the same assumption as in ordinary fixed-effect/random-effects meta-analysis,
  and typically reasonable if the sample size of the studies is not too small.}
Under the assumption of (asymptotic) normality, confidence intervals for
$\theta$ and $p$-values for testing the null hypothesis
...
rpcb <- read.csv("data/rpcb.csv") |>
    mutate(zo = smdo/so,
           ...
           dFormat = tableFormat(x = d, outFun = dFormat,
                                 threshold = 1, col1 = "dark2orange", col2 = "dark2green"),
           cioL = smdo - za*so,
           cioU = smdo + za*so,
           cirL = smdr - za*sr,
           cirU = smdr + za*sr,
           ...
           smdoFormat = paste("$", dFormat(smdo, phantom = TRUE), "~",
                              ciFormat(lower = cioL, upper = cioU), "$"),
           smdrFormat = paste("$", dFormat(smdr, phantom = TRUE), "~",
                              ciFormat(lower = cirL, upper = cirU), "$"),
           smdmFormat = paste("$", dFormat(smdm, phantom = TRUE), "~",
