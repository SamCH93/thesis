...
questions is of high importance to researchers, decision makers, and the general
population, and one way to obtain it is through \emph{replication studies}.

A replication study consists of repeating the original study as close as
...
Project'', see][for the distinction between ``replicabality'' and
...
one effect and one replication of that effect is included. For the three
replications at the bottom of the table (\#18, \#19, and \#20), all criteria
indicate replication failure. For the remaining majority of the studies, the
conclusions are more ambiguous; for no single replication study are all criteria
satisfied simultaneously. For instance, the first three studies satisfy the
significance criterion ($p_{o} < 0.05$ and $p_{r} < 0.05$), meta analytic
significance ($p_{m} < 0.05$) and the $Q$-test/prediction interval criterion
...
\begin{align*}
  ...
  \end{cases}
\end{align*}
similar to how ordinary one-sided $p$-values are defined. In the subsequent
chapters only the one-sided version of the sceptical $p$-value will be used.

The results from the ``Reproducibility Project: Cancer Biology'' in
Table~\ref{tab:RPCB} illustrate the properties of the sceptical $p$-value. For
example, study \#4 fails to achieve replication success at level
$\alpha = 2.5\%$ with the one-sided sceptical $p$-value, even though the
replication study was highly convincing (the effect estimate was almost five
times as large as in the original study). Yet, as the approach requires both
studies to be convincing on their own -- and the original study was not
significant at the 2.5\% level (one-sided) -- replication success at this level
is impossible with the sceptical $p$-value. The second property is illustrated
by study \#8. Here the original study was convincing and the replication study
also achieved significance. However, the effect estimate from the replication
was roughly 60\% smaller than the one from the original study, and significance
is merely achieved because the standard error was much smaller (\ie{} around
$1/\surd c \approx 1/2.8$ times smaller). The sceptical $p$-value is therefore
only $p_{\text{S}} = 0.12$, indicating hardly any evidence for replication
success.
...
The last part (Paper~\ref{PaperVI}) deals with meta-scientific perspectives of
...
the four replication projects, which I collected for my master thesis. L. Held
and C. Micheloud came up with the golden level. L. Held wrote an initial draft
of the manuscript. L. Held, C. Micheloud, and I then iteratively worked on the
manuscript.
...
fit nicely into this manuscript, to add the COVID-19 meta-analysis example, and
that I should start to overhaul it. I rewrote and expanded his initial draft,
adding also a new section on reverse-Bayes approaches with Bayes factors,
largely based on the work from Paper~\ref{PaperII}. We then managed to recruit R. Matthews
to also contribute. From that point on the three of us iteratively worked on the
manuscript and M. Ott gave high-level feedback.
...
Shortly after the acceptance of Paper~\ref{PaperIII}, the article by
...
evaluating ``how well'' statistical methods perform. The results of these
studies therefore often guide the choice in which situation which method should
...
L. Kook and K. Reeve. L. Kook and myself then inented the regression method
