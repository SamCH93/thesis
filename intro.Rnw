...
limits
\begin{align}
  \left[\mu_{\text{post}} - z_{\alpha/2} \, \sigma_{\text{post}},
  \mu_{\text{post}} + z_{\alpha/2} \, \sigma_{\text{post}}\right]
  \label{eq:postCI}
\end{align}
...
The last part (Paper~\ref{PaperVI}) is on meta-scientific perspectives on
