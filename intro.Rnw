...
is real? The answer to such questions is of highest importance to scientists and
decision makers, but typically we can never know for sure as any study result
...
original study with new subjects. Such a \emph{replication study} may yield
...
consequences, such as deciding whether we receive the recommendation to get
vaccinated (a consequence that anyone who experienced the COVID-19 pandemic is
very aware of).
...
prevent an escalation of the situation \citep[for an overview see
...
preregistration, preprints, and code and data sharing, all of which have
...
popularity, and several academic journals and funders are now explicitly
promoting and funding replication research \citep{NWO2016, NSL2018, Nature2022}.
...
\begin{table}[!htb]
  ...
  \caption{Study-level summary statistics for an original and a replication
    ...
    $\Phi^{-1}(1 - \alpha) = \z{\alpha}$. Confidence intervals and $p$-values
...
conducted since in this case replicators are often interested in also
understanding between-replication heterogeneity \citep[\eg{} in][]{Klein2018}.
...
followed by significance (\Sexpr{nsig}), and relative effect size
(\Sexpr{nres}). However, looking at the combinations of the criteria, only in
two replications are all of them satisfied simultaneously (green).
...
\begin{table}
  ...
  \caption{Subset of results from the Reproducibility Project: Cancer Biology
    ...
    relative effect estimate $d = \that_{r}/\that_{o}$, original $p$-value
    $p_{o}$, replication $p$-value $p_{r}$, meta-analytic $p$-value $p_{m}$,
    $Q$-test $p$-value $p_{Q}$, and sceptical $p$-value $p_{\mathrm{S}}$. A
    level $\alpha = 5\%$ is used as threshold for replication success via
    $p$-values, and a threshold $d_{\text{min}} = 1$ for the relative effect
    estimate.}
  \label{tab:RPCB}
...
revolves around the question of whether the resulting prior is plausible in
light of external knowledge, and if so, this could be seen as support for the
specified posterior.
...
evidence\footnote{For readers who do not agree with this notion of evidence, do
  not panic. In this thesis (Paper I and III) we extend this reverse-Bayes
...
\caption{Illustration of reverse-Bayes assessment of replication success using
  ...
  $\alpha = \Sexpr{100*(alpha)}\%$. Prior predictive $p$-values $p_{\text{Box}}$ are
...
approaches based on prior predictive ratios will be used in later parts of this
thesis.\footnote{Some readers may have noted that the prior predictive ratio is
  ...
  procedures based on Bayes factors, as will be demonstrated in later parts of
  this thesis (paper I and III).}
...
indicates less replication success. This property is desirable in the
...
which proposed a reverse-Bayes method. The last part (Paper VI) is about
integrity issues methodological research with simulations
studies. % The article lists and illustrates questionable
...
underlying both methods, yet in some situations conclusions may differ because
of their different ways of quantifying evidence. Specifically, it is shown that
the sceptical $p$-value suffers from a certain type of ``shrinkage paradox'',
which is avoided by the sceptical Bayes factor; when the $p$-value from the
original study goes to zero, replication success based on the sceptical
...
the world meeting of the ISBA (International Society for Bayesian Analysis) in
2021.
...
illustrate the properties of the method.
...
employed. We also give recommendations to alleviate these issues, most
importantly we recommend researchers write and pre-register simulation
protocols.
